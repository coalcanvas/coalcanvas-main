(function (window, document) {
  ...
  function validateUsername() {
    if (rules.userName.required) {
      ...
      if (userNameVal === "") {
        errors.push(
          `Username is <strong>required</strong>. You can't leave it empty.`
        );
        addErrorField(userNameEl);
      } else if (!isNaN(userNameVal.charAt(0))) {
        errors.push(
          `Username can't start with a number (<strong>${userNameVal.charAt(
            0
          )}</strong>)`
        );
        addErrorField(userNameEl);
        ...
        errors.push(
          `Username must be at least <strong>${rules.userName.minlength}</strong> characters.`
        );
        addErrorField(userNameEl);
      } else if (userNameVal.length > rules.userName.maxlength) {
        errors.push(
          `Username can contain maximum <strong>${rules.userName.maxlength}</strong> characters.`
        );
        addErrorField(userNameEl);
  ...
  function validateEmail() {
    if (rules.userEmail.required) {
      ...
      const searchSlangWord = rules.userEmail.slangWords.filter(function (
        slangWord
      ) {
      ...
      if (userEmailVal === "") {
        errors.push(
          `Email address is <strong>mandatory</strong>. You can't leave it empty.`
        );
        addErrorField(userEmailEl);
      } else if (!isEmailValid) {
        errors.push(
          `<strong>${userEmailVal}</strong> is not a valid email address.`
        );
        addErrorField(userEmailEl);
      } else if (!rules.userEmail.validEmailDomains.includes(userEmailDomain)) {
        errors.push(
          `<strong>${userEmailDomain}</strong> is not a valid email domain.`
        );
        addErrorField(userEmailEl);
      } else if (searchSlangWord.length) {
        errors.push(
          `Your email username contains slang word <strong>${searchSlangWord.join(
            ", "
          )}</strong>. Which we do not accept.`
        );
        addErrorField(userEmailEl);
      }
    }
  ...
  function validatePassword() {
    if (rules.userPassword.required) {
      ...
      if (!userPasswordVal.length) {
        errors.push(
          `Password is <strong>required</strong>. You can't leave it empty.`
        );
        ...
        errors.push(
          `Password must include at least <strong>1</strong> lowercase letter, <strong>1</strong> uppercase letter, <strong>1</strong> digit, and <strong>1</strong> special character.`
        );
        addErrorField(userPasswordEl);
      }
    }
  ...
  function validatePasswordConfirm() {
    ...
    if (
      signupForm.elements.userPassword.value.trim() !== userPasswordConfirmVal
    ) {
  ...
  function validatePhoneNumber() {
    if (rules.userPhoneNumber.required) {
      ...
      if (!userPhoneNumberVal.length) {
        errors.push(
          `Phone number is <strong>mandatory</strong>. You can't leave it empty.`
        );
        addErrorField(userPhoneNumberEl);
      } else if (userPhoneNumberVal.length < 11) {
        errors.push(
          `The phone number must be at least <strong>11</strong> digit long.`
        );
        addErrorField(userPhoneNumberEl);
      } else if (!rules.userPhoneNumber.pattern.test(userPhoneNumberVal)) {
        errors.push(
          `The phone number format is incorrect. We accept <strong>017-89-124-139</strong> or <strong>01789124139</strong> format.`
        );
        addErrorField(userPhoneNumberEl);
      }
    }
  ...
  function validateDOB() {
    if (rules.userDOB.required) {
      ...
      if (!userDOBVal.length) {
        errors.push(
          `Date of birth is <strong>required</strong>. You can't leave it empty.`
        );
        addErrorField(userDOBEl);
      } else if (!rules.userDOB.pattern.test(userDOBVal)) {
        errors.push(
          `The date format is incorrect. We accept <strong>21-Sep-1994</strong> in this format.`
        );
        addErrorField(userDOBEl);
      }
    }
      ...
      if (userPhotos.length === 0 || userPhoto === undefined) {
        ...
        errors.push(
          `The file you selected (${
            userPhoto.name
          }) is not a valid type. Valid file types are: <strong>${rules.userPhoto.accept.join(
            ", "
          )}</strong>.`
        );
        errorFields.push(userPhotoLabel);
        ...
        errors.push(
          `Your file size (${returnFileSize(
            userPhoto.size
          )}) is too large. The maximum allowed file size is <strong>100KB</strong>.`
        );
        errorFields.push(userPhotoLabel);
      }
    }
