...
function perspectiveHover(el, parent, intensity) {
  ...
  this.perspective = function (e, el) {
    ...
    el.style.webkitTransform =
      "rotateX(" + valX.toFixed(1) + "deg) rotateY(" + valY.toFixed(1) + "deg)"; // need webkit transform for this to work in safari 8
    el.style.transform =
      "rotateX(" + valX.toFixed(1) + "deg) rotateY(" + valY.toFixed(1) + "deg)"; // toFixed to round decimal values
  };
  ...
  this.anim = function (el, valX, valY, time) {
    animate({
      ...
      run: function (rate) {
        el.style.webkitTransform =
          "rotateX(" + rate * valX + "deg) rotateY(" + rate * valY + "deg)";
        el.style.transform =
          "rotateX(" + rate * valX + "deg) rotateY(" + rate * valY + "deg)";
      },
    });
